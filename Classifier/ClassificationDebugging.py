...
class VizClassifier():
    ...
    def getErrorImages(self, start, end, keyPath = None, host = None):
        ...
        if self.Classifier is not None:      
            ...
            print 'Start loading images on cloud server...'
            startTime = time.time()
            nImageAll = 0
            header = ['file_path']
            csvSavingPath = self.Opt.resultPath
            csvFilename = 'error_filenames'
            Common.saveCSV(csvSavingPath, csvFilename, header = header, mode = 'wb', consoleOut = False)
            ...
            endTime = time.time()
            print end - start, 'keys were gathered in ', endTime - startTime, 'sec'
            print 'Collection ends at key index = %d' % i
