...
class DictionaryExtractor:
    ...
    # Parameters
    Npatches = 50000;                   # Number of patches
    ...
    MIN_PATCH_VAR = float(38)/255       # Minimum Patch Variance for accepting as potential centroid (empirically set to about 25% quartile of var)
    MAX_TRY = 30                        # Maximum number of try to find a qualify patch from an image
    # Data
    ...
    def extractPatch(self, allImData, allLabels):
        ...
        nx, ny, nc = self.finalDim
        rf = self.rfSize;

        A = np.zeros((self.Npatches, rf * rf * nc))
        A = np.asmatrix(A)
        L = np.ones((self.Npatches, 1))
        L = np.asmatrix(L)
        ...
        maxTry = 0
        count_repeat = 0
        while i < self.Npatches:
            if trials % 10000 == 0:
                print i, '/', self.Npatches,  'patches accepted.'
            ...
            if np.var(patch) >= self.MIN_PATCH_VAR:
                ...
                i += 1
                maxTry = 0
            elif self.MAX_TRY > 0 and maxTry >= self.MAX_TRY:
                A[i,:] = np.reshape(patch, (rf * rf * nc), 'F')
                L[i] = allLabels[index]
                i += 1
                maxTry = 0
        ...
        print self.Npatches, 'patches extracted in', endTime - startTime, 'sec\n'
    ...
    def kmeansCentroids(self):
        ...
        self.centroids, self.centroidFrequency = DictionaryExtractor.kmeansVL(normPathces, self.Ncentroids, self.minibatch)
...
#         highlight = mod
        ...
        for i in range(self.Ncentroids):
            ...
            if NChannel > 1:
                image[int(r*(H+1)):int((r+1)*(H+1))-1, int(c*(W+1)):int((c+1)*(W+1))-1, :] = centr
            else:
                image[int(r*(H+1)):int((r+1)*(H+1))-1, int(c*(W+1)):int((c+1)*(W+1))-1] = centr
